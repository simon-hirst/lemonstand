...
const sendToQueue = async (routingKey, message) => {
  ...
  await channel.publish('payments', routingKey, Buffer.from(JSON.stringify(message)), {
    persistent: true
  });
  
  console.log(`Sent message to payments exchange with routing key: ${routingKey}`);
};
...
const consumeFromQueue = async (queueName, routingKey, callback) => {
  ...
  await channel.bindQueue(queue, 'payments', routingKey);
